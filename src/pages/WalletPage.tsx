...
export default function WalletPage() {
  ...
  const renderContent = () => {
    if (isLoading) {
      return (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <p style={{ color: colors.text.muted, fontSize: 14 }}>불러오는 중...</p>
        </div>
      );
    }
    ...
    if (isError) {
      return (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <p style={{ color: colors.text.danger, fontSize: 14 }}>
    ...
    if (!wallet?.principal) {
      return (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <p style={{ color: colors.text.muted, fontSize: 14 }}>활성화된 대출 한도가 없습니다.</p>
        </div>
      );
    }

    const { depositBankName, depositAccountNumber, balance, nextRepaymentDate, monthlyInterest, principal } = wallet;
    ...
    return (
      <>
        ...
        <div style={{ paddingLeft: 26, paddingTop: 30 }}>
          ...
          >
            {depositBankName} {depositAccountNumber}
          </p>
        </div>
        ...
        <div style={{ margin: '10px 24px 0', textAlign: 'right' }}>
          <p style={{ fontWeight: 700, fontSize: 14, lineHeight: '17px', color: colors.text.muted }}>
            다음 상환 예정일 : <span>{nextRepaymentDate ?? '-'}</span>
          </p>
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 13 }}>
              ...
              >
                <p style={{ fontWeight: 700, fontSize: 11, lineHeight: '13px', color: '#E65100' }}>
                  <span>{nextRepaymentDate ?? '-'}</span> 상환 예정
                </p>
              </div>
              <p style={{ fontWeight: 700, fontSize: 24, lineHeight: '29px', color: colors.text.danger }}>
                <span>{monthlyInterest !== null ? monthlyInterest.toLocaleString() : '-'}</span>
                {monthlyInterest !== null && <span style={{ fontSize: 20, marginLeft: 4 }}>원</span>}
              </p>
            </div>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 13 }}>
              ...
              >
                <p style={{ fontWeight: 700, fontSize: 11, lineHeight: '13px', color: '#878787' }}>
                  <span>{principal.dueDate}</span> 상환 예정
                </p>
              </div>
              <p
                style={{
              ...
              >
                <span>{principal.remainingAmount.toLocaleString()}</span>
                <span style={{ fontSize: 20, marginLeft: 4 }}>원</span>
              </p>
            </div>
          </div>
